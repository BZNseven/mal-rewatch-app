...
exports.handler = async (event) => {
  try {
    const path = event.path.replace("/.netlify/functions/mal-proxy", "");
    ...
    if (path.startsWith("/animelist")) {
      ...
      const url = qStr ? `${base}${qStr}` : `${base}?limit=100&fields=list_status{tags,score,status,is_rewatching,num_times_rewatched,rewatch_value}`;
    ...
    if (path.startsWith("/anime/")) {
      ...
      const url = qStr ? `${base}${qStr}` : `${base}?fields=genres,studios,start_season,num_episodes,media_type`;
